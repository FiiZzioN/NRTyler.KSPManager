...
namespace NRTyler.KSPManager.ServiceTests.UtilityTests
{
	...
	public class StringEnumTests 
	{
		...
		public void TestInstanceGetStringValues()
		{
			...
			Assert.AreEqual(0, stringEnum.GetStringValues().Count);

			stringEnum = new StringEnum(typeof(EnumPartialStrings));
			Assert.AreEqual(1, stringEnum.GetStringValues().Count);
			Assert.AreEqual("Jack be nimble", stringEnum.GetStringValues()[0]);
